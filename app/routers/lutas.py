...
from database.schemas.LutaSchema import LutaSchema

router = APIRouter()

@router.get("/", response_model=List[LutaSchema])
async def listar_lutas(
    ...
    metodo_finalizacao: Optional[str] = Query(None, description="Filtrar por método de finalização"),
    ...
    if categoria_luta:
        filters.append(Luta.categoria.ilike(f"%{categoria_luta}%"))
    if data_luta:
        filters.append(Luta.data == data_luta)
    if disputa_cinturao is not None:
        filters.append(Luta.disputa_cinturao == disputa_cinturao)
    if metodo_finalizacao:
        filters.append(Luta.finalizacao.ilike(f"%{metodo_finalizacao}%"))
    if nome_lutador:
