...
def update_fights(df_fights: pd.DataFrame, db: Session): 
    for _, row in df_fights.iterrows():
        try:
            # Obtém o lutador a partir do nome para adicionar na luta
            fighter_red = db.query(Lutador).filter(Lutador.nome_lutador == row['red_name']).first()
            if not fighter_red:
                print(f"Lutador não encontrado: {row['red_name']}")
                continue

            fighter_blue = db.query(Lutador).filter(Lutador.nome_lutador == row['blue_name']).first()
            if not fighter_blue:
            ...
            nova_luta = Luta(
                ...
                red_reversals=row['red_reversals'],
                ...
                blue_reversals=row['blue_reversals'],
            ...
            db.rollback() #Desfaz qualquer alteração na sessão em caso de erro.
